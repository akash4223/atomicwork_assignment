...
if (window.matchMedia("(max-width: 768px)").matches) //Mobile animation
mobileAnimation();
if (window.matchMedia("((min-width: 768px) and (max-width: 991px))").matches) //Tablet animation
tabletAnimation();
if (window.matchMedia("(min-width: 991px)").matches) //Desktop animation
desktopAnimation();
/////////////////////ROI CALCULATOR////////////////////////
...
calcSubBtn.addEventListener("click", (e)=>{
    ...
    let isEmpty = false;
    let calcFormFields = Array.from(document.querySelectorAll(".calc-form-field"));
    calcFormFields.forEach(function(field) {
        if (field.tagName == "INPUT" && field.value.trim() == "") isEmpty = true;
        else isEmpty = false;
    });
    if (isEmpty) {
        //throw error
        ...
        let moneySaved = totalResources.value * totalComp.value - AtomicWorkCostPerEmp * totalResources.value;
